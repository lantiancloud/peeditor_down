...
reset='\e[0m'
# 输出标记
log_info="${turquoise}INFO${reset} [peeditor_down]"
log_warn="${yellow}WARN${reset} [peeditor_down]"
log_error="${red}ERROR${reset} [peeditor_down]"
# 构建显示时间函数
update_time() {
...
if [ -z "${input}" ]; then
  ...
  update_time
  echo -e "${current_time} ${log_info} 正在注入LeviLamina"
  ./PeEditor.exe -mb
  update_time
  echo -e "${current_time} ${log_info} 注入完成"
  break
  exit 0
fi
# 如果是其他版本
